...
def main(): 
    ...
    随库图片前缀列表 = [os.path.splitext(f'{x[0]}/{x[1]}'.lstrip('/'))[0]
                for x in 随库图片列表]
    主库需要压缩的图片列表 = [y for y in 主库图片列表  
                   if os.path.splitext(f'{y[0]}/{y[1]}'.lstrip('/'))[0] 
                   not in 随库图片前缀列表]
    ...
    随库视频前缀列表 = [os.path.splitext(x)[0] 
                for x in 随库完好视频列表]
    主库需要压缩的视频列表 = [x for x in 主库视频列表
                   if os.path.splitext(f'{x[0]}/{x[1]}'.lstrip('/'))[0] 
                   not in 随库视频前缀列表]
    ...
    print(f'    有 {len(随库图片列表)} 个图片，其中有 {len(随库需要删除图片列表)} 个需要删除')
    print(f'    有 {len(随库视频列表)} 个视频，其中有 {len(随库需要删除视频列表)} 个需要删除')
...
def 筛选出完好的视频列表(根目录, 列表, 记录文件):
    ...
    记录文件 = os.path.join(根目录, 记录文件)
    if not os.path.exists(记录文件):
    ...
    with open(记录文件, encoding='utf-8') as f:
        ...
        待测列表 = [x for x in 列表 if os.path.join(x[0], x[1]) not in 记录列表]
    
    # 对没有记录的视频进行检测
    with open(记录文件, 'a', encoding='utf-8') as f:
        for 待测项 in 待测列表:
            相对路径 = os.path.join(待测项[0], 待测项[1])
            if 判断视频是否已压制完成(os.path.join(根目录, 相对路径)):
                记录列表.append(相对路径)
                f.write(相对路径 + '\n')
...
def 依列表删除随库文件(文件列表):
    # 列表中包含的项是这样的： ['relative/path', 'file.name']
    for item in 文件列表:
        完整路径 = os.path.join(随库位置, item[0], item[1])
        try:
            os.remove(完整路径)
...
def 压缩主库视频到随库(视频列表):
    ...
    for index, item in enumerate(视频列表):
        ...
        with open(os.path.join(随库位置, 完好视频记录文件), 'a', encoding='utf-8') as f:
            f.write(f'{dst_rel}\n')
